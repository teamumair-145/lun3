...
# only /tmp is writable, and it's wiped between invocations. Detect that and
# use /tmp there; keep a local ./uploads folder for normal servers.
...
# NOTE: Vercel's serverless functions reject request bodies above ~4.5 MB
# regardless of this setting, so large videos will fail there no matter what
# MAX_CONTENT_LENGTH says. Keep this high for other hosts (PythonAnywhere,
# Render, Railway, your own server); see DEPLOY_VERCEL.md for details.
